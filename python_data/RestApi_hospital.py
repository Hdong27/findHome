...
def hospital_score(address):
    ...
    while(True):
        ...
        # print(category_name)
        # print(distance)
        # print(min(distance)) # 가장 가까운 병원까지 거리
        # print(len(category_name)) # 반경내에 병원 수
        is_end = r.json()["meta"]["is_end"]
        # with open('jsonFile\\hospital'+str(n)+'.json', 'w', encoding='utf-8') as make_file:
        #     json.dump(r.json(), make_file, ensure_ascii=False, indent="\t")
        if is_end:
            break
        n+=1
    return query,totalCnt