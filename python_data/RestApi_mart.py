...
def mart_score(address):
    ...
    while(True):
        ...
        size=15
        r = requests.get(url, params = {'category_group_code':category_group_code,'radius':radius,'page':page,'size':size,'x':x,'y':y}, headers={'Authorization' : 'KakaoAK ' + apikey } )
        r.json()
        # print(r.json())
        ...
        # print(distance)
        # print(min(distance)) # 가장 가까운 마트까지 거리
        is_end = r.json()["meta"]["is_end"]
        with open('jsonFile\\mart'+str(n)+'.json', 'w', encoding='utf-8') as make_file:
            json.dump(r.json(), make_file, ensure_ascii=False, indent="\t")
        if is_end:
            break
        n+=1
    # print(min(distance))  # 가장 가까운 편의점까지 거리
    return "마트", min(distance), totalCnt